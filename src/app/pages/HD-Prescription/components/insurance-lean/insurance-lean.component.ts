...
@Component({
  selector: 'arxrf-insurance-lean',
  templateUrl:
    '../../../home-delivery/components/insurance-lean/insurance-lean.component.html',
  styleUrls: [
    '../../../home-delivery/components/insurance-lean/insurance-lean.component.scss'
  ]
})
export class InsuranceComponent implements OnInit {
  ...
  cancelAction() {
    ...
    if (
      ...
      this.redirectToRoute(
        ROUTES.hd_prescription.children.enroll_insurance.absoluteRoute
      );
    } else {
  ...
  getInsuranceInfo(memId) {
    ...
    // if ( this.isBuyout ) {
    //   activeMember = this._userService.getActiveMemberId();
    // }
  ...
  addInsurancePrefillUserInfo() {
    ...
    const activeMemberFullName =
      userInfo['firstName'].charAt(0).toUpperCase() +
      userInfo['firstName'].slice(1) +
      ' ' +
      userInfo['lastName'].charAt(0).toUpperCase() +
      userInfo['lastName'].slice(1);
    this.user = activeMemberFullName;
    this.insuranceInformationForm.patchValue({
      // tslint:disable-next-line: max-line-length
      cardholderName: activeMemberFullName,
